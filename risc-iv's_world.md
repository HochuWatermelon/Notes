...
p.s. Все-таки самым логичным способом инициализировать верхушку стека является тот, что ниже, так как в тестах у нас заводится лимит памяти 0x1000, стек растет вниз, поэтому логично его верхушку завести как можно дальше от секции text, поэтому ставим ее в самый конец.
```
	lui     sp, %hi(0x1000)                
    	addi    sp, sp, %lo(0x1000)
```
![image](https://github.com/user-attachments/assets/d251e200-aee0-4882-a376-8781eaf18713)

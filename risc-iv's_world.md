...
Пояснение:
Мы совершаем переход к метке, при этом сохраняем значение адреса следующей команды в регистре (адрес возврата). Вспоминаем прерывания или вызов функции. Вызываем обработку функции и после ее завершения необходимо вернуться на тоже место, откуда была вызвана функция, и перейти к следующей строке программы.

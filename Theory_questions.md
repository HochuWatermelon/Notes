...
Адреса input port и output port не предназначены для исполнения из них инструкций, а только для обмена данными. Поэтому на аппаратном уровне нам нужно запрещать процессору исполнять из этих адресов инструкции, то есть когда счетчик команд PC достигнет этого адреса, нужно вызвать исключение/ошибку. Это и происходит, кста, выглядит таким образом:
